...
// StoreAutoReply creates or updates an auto-reply rule
func (r *SQLiteRepository) StoreAutoReply(reply *domainChatStorage.AutoReply) error {
	now := time.Now()
	...
	if reply.ID == 0 {
		// Insert
		result, err := r.db.Exec(`
			INSERT INTO auto_replies (keyword, response, match_type, created_at)
			VALUES (?, ?, ?, ?)
		`, reply.Keyword, reply.Response, reply.MatchType, reply.CreatedAt)
		if err != nil {
			return err
		}
		...
		// Update
		_, err := r.db.Exec(`
			UPDATE auto_replies SET keyword = ?, response = ?, match_type = ?
			WHERE id = ?
		`, reply.Keyword, reply.Response, reply.MatchType, reply.ID)
		if err != nil {
			return err
		}
	}
...
// GetAutoReplies retrieves all auto-reply rules
func (r *SQLiteRepository) GetAutoReplies() ([]*domainChatStorage.AutoReply, error) {
	rows, err := r.db.Query(`
		SELECT id, keyword, response, match_type, created_at
		FROM auto_replies
	...
	var replies []*domainChatStorage.AutoReply
	for rows.Next() {
		var reply domainChatStorage.AutoReply
		if err := rows.Scan(&reply.ID, &reply.Keyword, &reply.Response, &reply.MatchType, &reply.CreatedAt); err != nil {
			return nil, err
		}
...
// GetAutoReply retrieves a single auto-reply rule
func (r *SQLiteRepository) GetAutoReply(id int64) (*domainChatStorage.AutoReply, error) {
	var reply domainChatStorage.AutoReply
	err := r.db.QueryRow(`
		SELECT id, keyword, response, match_type, created_at
		FROM auto_replies
		WHERE id = ?
	`, id).Scan(&reply.ID, &reply.Keyword, &reply.Response, &reply.MatchType, &reply.CreatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	return &reply, err
...
func (r *SQLiteRepository) DeleteAutoReply(id int64) error {
	_, err := r.db.Exec("DELETE FROM auto_replies WHERE id = ?", id)
	return err
}
